...
// -- GET /api/barbers
export const getBarbers = (req, res) => {
...
// -- GET /api/promotions
export const getPromotions = (_req, res) => {
...
export const getReviews = (req, res) => {
    ...
    db.get("SELECT COUNT(*) as count FROM reviews WHERE approved=1", [], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        const totalItems = row.count;
        const totalPages = Math.ceil(totalItems / limit);
        db.all("SELECT * FROM reviews WHERE approved=1 ORDER BY createdAt DESC LIMIT ? OFFSET ?", [limit, offset], (err2, rows) => {
            if (err2)
...
export const createReview = (req, res) => {
    ...
    db.run("INSERT INTO reviews (clientName, rating, comment, approved) VALUES (?, ?, ?, 0)", [clientName, rating, comment || ""], function (err) {
        if (err)
            return res.status(500).json({ error: err.message });
...
export const getBlockedSlots = (req, res) => {
...
export const getBookingByCode = (req, res) => {
...
export const createBooking = (req, res) => {
    ...
    db.get("SELECT durationMin FROM services WHERE id=?", [serviceId], (err, row) => {
        if (err)
            return res.status(500).json({ error: err.message });
        if (!row)
