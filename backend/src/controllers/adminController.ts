...
export const getPromotionsAdmin = (_req: Request, res: Response) => {
...
export const getReviewsAdmin = (req: Request, res: Response) => {
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 10;
    const offset = (page - 1) * limit;

    db.get("SELECT COUNT(*) as count FROM reviews", [], (err, row: any) => {
        if (err) return res.status(500).json({ error: err.message });
        const totalItems = row.count;
        const totalPages = Math.ceil(totalItems / limit);

        db.all(
            "SELECT *, approved as isApproved FROM reviews ORDER BY createdAt DESC LIMIT ? OFFSET ?",
            [limit, offset],
            (err2, rows) => {
...
export const setReviewNote = (req: Request, res: Response) => {
    const { id } = req.params;
...
export const getDashboardStats = (_req: Request, res: Response) => {
    ...
    db.get(sqlPending, (e1, r1: any) => {
        if (e1) return res.status(500).json({ error: e1.message });
        db.get(sqlToday, (e2, r2: any) => {
            if (e2) return res.status(500).json({ error: e2.message });
            db.get(sqlWeek, (e3, r3: any) => {
                if (e3) return res.status(500).json({ error: e3.message });
                db.get(sqlMonth, (e4, r4: any) => {
...
export const getBookings = (req: Request, res: Response) => {
  ...
  `;
    ...
    // 1. Count Total
    db.get(`SELECT COUNT(*) as count ${baseSql}`, params, (err, row: any) => {
        if (err) return res.status(500).json({ error: err.message });
        const totalItems = row.count;
        const totalPages = Math.ceil(totalItems / limit);
...
export const updateBookingStatus = (req: Request, res: Response) => {
    ...
    db.get("SELECT * FROM bookings WHERE id=?", [id], (err, row: any) => {
        if (err) return res.status(500).json({ error: err.message });
...
// -- SERVICES CRUD --
export const getServicesAdmin = (_req: Request, res: Response) => {
...
export const createService = (req: Request, res: Response) => {
    ...
    db.run(
        "INSERT INTO services (title, durationMin, price, category, photo) VALUES (?,?,?,?,?)",
        [title, durationMin, price, category, photo],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
...
// -- BARBERS CRUD --
export const getBarbersAdmin = (_req: Request, res: Response) => {
...
export const createBarber = (req: Request, res: Response) => {
    ...
    db.run(
        "INSERT INTO barbers (name, role, photo) VALUES (?,?,?)",
        [name, role, photo],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ id: this.lastID, success: true });
...
// -- GALLERY CRUD --
export const getGalleryAdmin = (_req: Request, res: Response) => {
...
export const uploadGallery = (req: Request, res: Response) => {
    const photo = req.file ? `/uploads/gallery/${req.file.filename}` : null;
    if (!photo) return res.status(400).json({ error: "No file" });
...
export const deleteGallery = (req: Request, res: Response) => {
...
export const createPromotion = (req: Request, res: Response) => {
    ...
    db.run(
        "INSERT INTO promotions (title, description, photo, expiresAt, price) VALUES (?,?,?,?,?)",
        [title, description, photo, expiresAt || null, price],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
            res.json({ id: this.lastID, success: true });
        }
